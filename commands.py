...
def handle_admin_upload(chat_id: int, user_id: int, document: Optional[Dict[str, Any]], video: Optional[Dict[str, Any]]) -> None:
    if user_id != ADMIN_ID:
        ...
        return
    file_id = document['file_id'] if document and 'file_id' in document else video['file_id'] if video and 'file_id' in video else None
    if file_id:
...
def handle_announce_callback(chat_id: int, user_id: int, callback_data: str, callback_query: Dict[str, Any]) -> None:
    ...
    if callback_data.startswith("announce_confirm_"):
        ...
        for user in users:
            uid = user['user_id']
            try:
                if send_message(uid, f"📢 Announcement:\n{message}"):
                    success_count += 1
            except Exception:
                continue
