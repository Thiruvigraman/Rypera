...
def health():
    try:
        ...
        log_to_betterstack("health_check", {"uptime": uptime, "memory_mb": mem, "cpu_percent": cpu})
        ...
        log_to_betterstack("health_endpoint_error", {"error": str(e)})
...
def handle_webhook():
    try:
        ...
        log_to_betterstack("webhook_error", {"error": str(e)})
...
def shutdown():
    if request.json.get('admin_id') == str(ADMIN_ID):
        ...
        log_to_betterstack("bot_shutdown", {"admin_id": ADMIN_ID})
        os._exit(0)
...
def monitor_resources():
    while not is_shutting_down:
        ...
        mem = process.memory_info().rss / 1024 / 1024
        cpu = process.cpu_percent(interval=0.1)
        log_to_betterstack("resource_usage", {"memory_mb": mem, "cpu_percent": cpu})
...
log_to_betterstack("bot_startup", {})
try:
    cleanup_pending_files()
except Exception as e:
    log_to_discord(DISCORD_WEBHOOK_STATUS, f"Startup cleanup error: {str(e)}", log_type='status', severity='error')
    log_to_betterstack("startup_cleanup_error", {"error": str(e)})

def on_exit():
    if is_shutting_down:
        log_to_discord(DISCORD_WEBHOOK_STATUS, "Bot turned off", log_type='status', severity='info')
        log_to_betterstack("bot_exit", {})

def handle_shutdown(signum, frame):
    global is_shutting_down
    is_shutting_down = True
    log_to_discord(DISCORD_WEBHOOK_STATUS, "Bot turned off", log_type='status', severity='info')
    log_to_betterstack("bot_shutdown_signal", {"signal": signum})
    os._exit(0)
...
if __name__ == '__main__':
    try:
        ...
        log_to_betterstack("flask_crash", {"error": str(e)})
        raise