import os import json import requests import threading import atexit from flask import Flask, request, jsonify from pymongo import MongoClient from dotenv import load_dotenv
...
ADMIN_ID = int(ADMIN_ID) TEMP_FILE_IDS = {}

MongoDB Setup

client = MongoClient(MONGODB_URI) db = client['telegram_bot'] movies_collection = db['movies']

Webhook Logger

def log_to_discord(webhook, message): if webhook: requests.post(webhook, json={"content": message})
...
def load_movies(): movies = {} for doc in movies_collection.find(): movies[doc['name']] = {"file_id": doc['file_id']} return movies
...
def send_file(chat_id, file_id): url = f'https://api.telegram.org/bot{BOT_TOKEN}/sendDocument' payload = {'chat_id': chat_id, 'document': file_id} response = requests.post(url, json=payload) message_data = response.json()
...
def delete_message(chat_id, message_id): url = f'https://api.telegram.org/bot{BOT_TOKEN}/deleteMessage' payload = {'chat_id': chat_id, 'message_id': message_id} requests.post(url, json=payload)
